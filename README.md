...
The extension stores only tab IDs in `chrome.storage.session` and the chosen shortcut in local extension storage. It has no Switchy server or analytics. For the visible favicon row, it uses Chrome's internal favicon endpoint rather than asking the active webpage to load an external image; local/private/internal tabs use a local fallback. It has an always-ready local content script so modifier release is captured without injecting code mid-gesture. Chrome does not permit extension scripts or their keyboard listeners on `chrome://` pages, the Chrome Web Store, or other protected browser pages. Close those tabs and test the shortcut on a regular website.
